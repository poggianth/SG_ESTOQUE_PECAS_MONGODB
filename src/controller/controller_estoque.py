...
class Controller_Estoque:
    ...
    def existe_estoque(self, id_estoque: int):
        self.mongo.connect()
        result = self.mongo.db[self.collection_name].find_one({"codigo": id_estoque})
    ...
    def inserir_estoque(self):
        ...
        try:
            qtd_estoque = self.mongo.db[self.collection_name].count_documents({})
            self.mongo.db[self.collection_name].insert_one({"tipo": tipo_estoque, "codigo": qtd_estoque+1})
    ...
    def alterar_estoque(self):
        if relatorio.get_estoque_todos_estoques():
            codigo_estoque_alterar = int(input("\nInforme o código(id) do estoque que irá ALTERAR: "))

            try:
                self.mongo.connect()
                estoque_existente = self.mongo.db["estoques"].find_one({"codigo": codigo_estoque_alterar})

                if estoque_existente != None:
                    tipo_estoque_novo = input("Informe o (NOVO) tipo: ")

                    # Atualiza o tipo do estoque
                    self.mongo.db["estoques"].update_one({"codigo": codigo_estoque_alterar}, {"$set": {"tipo": tipo_estoque_novo}})
                    print(f"Estoque({codigo_estoque_alterar}) alterado com sucesso!")
                    ...
                    print(f"Não existe nenhum estoque com o código(id) = {codigo_estoque_alterar}")
    ...
    def excluir_estoque(self):
        if relatorio.get_estoque_todos_estoques():
            codigo_estoque_excluir = int(input("\nInforme o código(id) do estoque que irá EXCLUIR: "))

            try:
                self.mongo.connect()

                if self.existe_estoque(codigo_estoque_excluir):
                    dependentes_encontrados = self.mongo.db["itens_estoque"].find({"codigo_estoque": codigo_estoque_excluir})
                    df_dependentes_encontrados = pd.DataFrame(dependentes_encontrados)
                    possuiDependentes = False

                    if not df_dependentes_encontrados.empty:
                        possuiDependentes = True
                        print(f"\nForam encontrados produtos armazenados no estoque ({codigo_estoque_excluir}):\n")
                    ...
                    if opcao_certeza == 1:
                        if possuiDependentes:
                            self.mongo.db["itens_estoque"].delete_many({"codigo_estoque": codigo_estoque_excluir})

                        self.mongo.db["estoques"].delete_one({"codigo": codigo_estoque_excluir})

                        print(f"Estoque({codigo_estoque_excluir}) excluído com sucesso!")
                    elif opcao_certeza == 0:
                        print("Operação de exclusão cancelada com sucesso!")
                    else:
                        print("Opção inválida!")
                else:
                    print(f"Não existe nenhum estoque com o código(id) = {codigo_estoque_excluir}")
